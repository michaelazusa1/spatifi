...
function getSearchHTMLPage(body, _gl, _hl) {
    ...
    let data = findJSON('search.html', 'search_response', body, /\bytInitialData\s*=\s*{/, '</script>', '{');
...
function toPlaylist(item) {
    try {
        ...
        return {
            ...
            image: thumbnails ? thumbnails[0].thumbnails.slice(-1)[0].url : thumbnail.thumbnails.slice(-1)[0].url,
            playlistId,
            count: videoCountShortText ? videoCountShortText.runs[0].text : videoCountText.runs[0].text,
            description: longBylineText.simpleText ? longBylineText.simpleText : longBylineText.runs[0].text
        };
    } catch (error) {
